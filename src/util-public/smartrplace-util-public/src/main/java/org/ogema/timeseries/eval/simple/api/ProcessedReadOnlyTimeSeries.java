...
/**
 * Implementation for {@link ReadOnlyTimeSeries} that is based on some input
 * provided via {@link #updateValues(long, long)}. The class requests any
 * unknown values from the implementation via this method. In in standard
 * operation it assumes that for a given interval all values in the interval are
 * returned on the first request and that the values in the interval do not
 * change. The class has been extended, though, to take care of changes in input
 * already processed:<br>
 *  * By setting knownEndUpdateInterval intervals that go beyond the current
 * time or that range into a current aligned interval (like the current day) are
 * reprocessed regularly (see next Section on on this).<br>
 *  * By providing data in {@link #addIntervalToUpdate(DpGap)} or overwriting
 * {@link #getIntervalsToUpdate(long, long)} intervals can be provided that need
 * recalculation e.g. because of manual input<br>
 * Note that if knownEndUpdateInterval is null the limits of values provided via
 * updateValues and the data within intervals that have been requested before
 * are NOT updated later on. If the interval is set then after the interval is
 * finished the knownEnd is reset to the time of the last update meaning that it
 * is assumed that everything behind this last update is unknown.<br>
 * In practice the caching mechanism can lead to several problems. It is still
 * decisive to be able to process a larger quantity of input data. If you set
 * knownEndUpdateInterval to a very short time (e.g. 10 seconds) and
 * absoluteTiming to AbsoluteTiming.ANY_RANGE then almost no caching should take
 * place anymore (to be tested).<br>
 * <br>
 * There are also several ways to limit the frequency of recalculations to avoid
 * performance issues:<br>
 *   * property org.ogema.timeseries.eval.simple.api.knownIntervalUpdate sets
 * knownEndUpdateInterval to the value (in seconds). Take effect on all
 * instances of ProcessedReadOnlyTimeseries if not set explicitly otherwise in
 * the constructor (which is not used at the time of this writing). The default
 * value is 7200 (2 hours, usually this has to be set to a shorter interval) *
 * property
 * org.ogema.timeseries.eval.simple.api.intervalsToUpdateProcessingInterval sets
 * the limit for processing getIntervalsToUpdate. Also takes effect on all
 * ProcessedReadOnlyTimeseries. If null all changes are processed immediately
 * (default). * Setting *minIntervalForReCalc* in the constructor of an instance
 * of TimeseriesSimpleProcUtil, which is put forward into the instances of
 * ProcessedReadOnlyTimeSeries generated by it. Inside the limit given here the
 * timeseries returns just the values calculated before and does not check for
 * any updates or recalculations. This is an effective limit in some online
 * situations when the source data is written very frequently, but can lead to
 * unexpected behavior with complex interdependencies of timeseries. So this
 * cannot be set for all timseries with a single property. * Note that the
 * property flag evaldebug0 is especially foreseen to enable performance
 * debugging
 * <br>
 * Debugging:<br>
 * Especially the page with TS-ANY Datapoints is important to see the current
 * status of datapoints for which timeseries are calculated via this evaluation.
 * To track the call stacks of the evaluation you should enable the property
 * "evaldebug", which leads to console outputs with a label usually identifiying
 * the time series that is processed at this point. If the result is written
 * into slotsDB then take care of Debugging issues with Virtual SlotsDB Data.
 * <br>
 * Background with Open Issues:<br>
 * TODO 1: Currently for all data provided via {@link #updateValues(long, long)}
 * is checked whether the result overlaps with existing data and any existing
 * data in the range is deleted if this is the case. This avoids that the
 * internal list of SampledValues ({@link #values}) gets unordered etc. In some
 * cases this may not be necessary and very resource-costly, but in most typical
 * cases this is the intended behavior.<br>
 * TODO 2: The caching management is especially critical for
 * {@link TimeseriesSetProcMultiToSingle} that take several time series as
 * input, oftentimes ProcessedReadOnlyTimeSeries themselves. Currently we
 * operate all such time series in updateMode=4, which means that any reported
 * change in the input data will lead to a complete recalculation. In typical
 * online operation this is usually no problem and a reasonable trade-off to
 * manage the caching issues, but in offline evaluation or in larger settings
 * (especially if a lot of high resolution metering data is collected over time)
 * this may need a change. For input time series that do not generate a note on
 * their datapoint when data changes it is still an issue to make sure that any
 * new data is taken care of (now usually by the absoluteTiming parameter).<br>
 * TODO 3: If the result is aligned and the full input data is available from
 * the beginning the calculation may be not very efficient. When on aligend
 * result is calculated taking into account all input data until the next
 * aligned interval then knownEnd may be set to some time at the result
 * currently calculated or somewhere shortly behind unaligned, but before the
 * next aligned result that may be requested by the upper level evaluation in
 * the next step. Then the previous aligned interval result is calculated once
 * again together with the new result. If the base input is flowing in via data
 * logging then this behavior may make sense as the last aligned aggregated
 * value is updated all the time until the next aligned interval begins. NOTE
 * that in many cases this is not an issue as the entire time range is usually
 * calculated e.g. when the size method is called. For large offline evaluations
 * this issue has to be considered, though.<br>
 * <br>
 * Background information:<br>
 * Access synchronization: The getValues method should not be processed for the
 * same object of type {@link ProcessedReadOnlyTimeSeries} twice in parallel as
 * this would trigger a double recalculation and setting the known limits would
 * be quite unpredictable. This requires that the implementation of updateValues
 * does not read the time series again via getValues. It could read via
 * {@link #getValuesWithoutUpdate(long, long)}, but this is without update, of
 * course. We have implemented this with a ReentrantLock. For the proc and cons
 * of ReentrantLock see the table in
...
public abstract class ProcessedReadOnlyTimeSeries implements ReadOnlyTimeSeries {
     ...
     * Constructor
     *
     * @param interpolationMode currently only InterpolationMode.NONE is
     * supported
     * @param knownEndUpdateInterval if not null this specifys a duration after
     * which the interval for which the time series values are assumed to be
     * known is reset to lastKnownEndUpdate. If absoluteInterval is set then it
     * will even be set back to the beginning of the interval of
     * lastKnownEndUpdate
     */
    public ProcessedReadOnlyTimeSeries(InterpolationMode interpolationMode,
    ...
    public List<SampledValue> getValues(long startTime, long endTime) {
        ...
        if (!isFree) {
            ...
            if (lockLog != null) {
                lockLog.logEvent((endOfAgg - startWait), "Acquired lock for " + dpLabel() + " after");
            }
        }
...
//logger.error("Found new vals:"+values.size());
    ...
    protected List<SampledValue> getValuesWithoutUpdate(long startTime, long endTime) {
        ...
        if (endTime == startTime) {
            ...
            for (int i = values.size() - 1; i >= fromIndex; i--) {
        ...
        long endOfAgg = getCurrentTime();
        if (subTsBuildLog != null) {
            subTsBuildLog.logEvent((endOfAgg - startCalc), "Calculation of SUB " + dpLabel() + " took");
        }
        return result;
    }
    ...
    protected void addValues(List<SampledValue> newVals) {
        ...
        if (subTsBuildLog != null) {
            subTsBuildLog.logEvent((endCalc - startCalc), "Calculation of ADD " + dpLabel() + " took");
        }
    }
    ...
    /**
     * Set back knownEnd. This should be used if the values shall be
     * re-calculated from a certain time.<br>
     * TODO: This method is NOT Thread-safe
     *
     * @param newKnownEnd
     * @param force if true it is possible to set knownEnd to a later time. This
     * means that for the increased intervals no calculations will be performed,
     * which usually is not intended
     ...
     *
     * @param reCalcUntil if not null a recalculation from zero until this time
     * will be triggered immediately, otherwise on the next call to getValues
     * @return
     */
    ...
    /**
     * TODO: In the future separate updateIntervals should be stored and
     * returned in {@link #getIntervalsToUpdate(long)}
     *
     * @param newInterval
     * @return
     */
    ...
    /**
     * Get all gaps to be filled since last call of this method. The intervals
     * should not be overlapping as we do not test on this here - this would
     * lead to a double re-calculation.<br>
     * NOTE: All values that shall be replaced MUST be overwritten using this
     * method. Do not try to overwrite a value just providing the same time
     * stamp again via {@link #updateValues(long, long)} as this will just add
     * two entries for the same time stamp or lead to badly ordered time stamps.
     */
